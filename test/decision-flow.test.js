...
test("queued stale answer clicks after a result are ignored", () => {
  const app = loadApp();
  ...
  click({ target: app.optionButtons[0] });
  app.runTimers();
  const finalAnswer = app.optionButtons[0];

  click({ target: finalAnswer });
  assert.match(resultCard.innerHTML, /Siinus rütm/);

  app.runTimers();
  assert.doesNotThrow(() => click({ target: finalAnswer }));
  assert.match(resultCard.innerHTML, /Siinus rütm/);
  assert.doesNotMatch(breadcrumb.innerHTML, /undefined/);
});
